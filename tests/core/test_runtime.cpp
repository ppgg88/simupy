...
void testPacerAbsorbsOneSlowStep() {
    ...
    RealTimePacer pacer(0.0, 10.0);

    const double elapsed = millisecondsFor([&pacer] {
        // Overrun the first four steps at once: absolute deadlines absorb it.
...
void testPacerReportsLag() {
    ...
    // Past the threshold the pacer gives up catching up.
...
class Counted(HardwareBlock):
    ...
    // Once more for the next run: a second setup must not reopen the port.
    checkClose(runToEnd(model), 2.0, 1e-12,
               "and once again for the second run");

    // Through a third block, since terminate() runs after the last sample.
    Model probe;
...
void testUdpLoopback() {
    ...
    // Paced: Windows delivers loopback asynchronously.
