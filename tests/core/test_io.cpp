...
/// Constant -> Gain -> Scope, with the gain named and labelled, so a copy has
/// something to lose if it is incomplete.
struct Chain {
    Block* source;
    Block* gain;
    Block* sink;
...
/// A pasted selection must keep its wiring, its parameters and its labels, and
/// must not collide with what is already there.
void testCopyPasteRoundTrip() {
    ...
    // The wire between the two copies must exist, and must join the copies
    // rather than reaching back into the originals.
...
/// A wire with one end outside the selection has nothing to reconnect to, so
/// it must be dropped rather than dangle.
...
/// Copying a subsystem has to take its contents, or the paste is an empty box.
void testCopySubsystemTakesContents() {
    ...
    // The copy has to be usable, not merely present.
...
/// Pasting the same clipboard twice must produce two independent copies.
void testPasteTwice() {
    ...
    // Every name in the model has to stay unique, because that is what the
    // logged signal paths are built from.
...
/// Nonsense on the clipboard must be refused rather than half-applied.
