...
/// Constant -> Integrator -> Scope: a ramp, so how far the run got is
/// readable from the last logged value.
...
/// The stop time must stop mattering, and nothing else may change.
...
/// The only way an unbounded run ends is by being asked to.
...
/// Without a span to divide, the automatic step bound has to come from
/// somewhere — otherwise it would be infinite and the first step endless.
...
/// A run with no end must not have a log with no end. Decimation is what
/// makes "run forever" a thing you can actually do.
void testUnboundedLogStaysBounded() {
    ...
    model.solver().unbounded = true;
    model.solver().maxStep = 0.01;
    // A small cap so the halving is reached quickly rather than after a
    // million samples.
    ...
    // Decimation must keep the shape, not just the count: the log still has
    // to span the whole run. It thins the samples, so the newest kept one can
    // lag the present by up to one decimation stride — the test allows for
    // that rather than pretending the last step is always kept.
...
/// An unbounded run has no stop time to validate, so the usual complaint about
/// one must not fire.
void testUnboundedSkipsStopTimeCheck() {
    ...
    model.solver().stopTime = 0.0;  // would be rejected on a bounded run
...
/// The setting travels with the model.
...
/// The whole point: a value changed part-way through a run has to reach the
/// blocks downstream, without stopping and recompiling.
void testControlChangesMidRun() {
    ...
    // Halfway through, move the slider — exactly what the panel does, only
    // from this thread rather than the interface one.
...
/// A toggle and a button have to produce their declared values, and the
/// button has to be momentary rather than latching.
void testToggleAndButtonValues() {
    ...
    // A toggle remembers; committing writes the state back so the model can
    // be saved with the switch where the user left it.
    toggleBlock->commitValue(-1.0);
    check(!toggle->params().boolean("on"), "committing off stores the state");

    // A button springs back, so its resting value is the only one worth
    // saving — committing the pressed value must not latch it.
...
/// Setting a control and *then* pressing Run must start from that setting.
///
/// Seeding the live value at setup is what makes a headless run use the saved
/// parameters at all — but doing it unconditionally would throw away exactly
/// the gesture of flipping a switch before starting.
void testControlSetBeforeRunSurvives() {
    ...
    // Flipped on before the run, exactly as clicking the block would.
    interactive->setLiveValue(5.0);
    checkClose(runToEnd(model), 5.0, 1e-12,
               "the run starts from what the control was set to");

    // A model nobody has touched still takes its value from the file — the
    // headless case, where no interface ever seeds anything.
    Model fresh;
...
/// Controls have to survive a save, or a tuned model would come back at its
/// defaults.
...
/// A reversed range is a typo. It must not produce a control that cannot move.
...
/// A loop closed through a wireless link is still an algebraic loop, and must
/// be solved rather than quietly mis-wired.
