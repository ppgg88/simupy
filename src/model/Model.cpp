...
void Model::removeBlock(const std::string& blockId) { takeBlock(blockId); }

BlockPtr Model::takeBlock(const std::string& blockId) {
    connections_.erase(
    ...
    auto it = std::find_if(blocks_.begin(), blocks_.end(),
                           [&](const BlockPtr& b) { return b->id() == blockId; });
    if (it == blocks_.end()) return nullptr;

    BlockPtr taken = std::move(*it);
    blocks_.erase(it);
    return taken;
}
