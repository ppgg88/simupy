...
struct Scope {
    ...
    Block* subsystem = nullptr;  ///< the Subsystem block in the parent model
    std::string prefix;
};
...
private:
    /// Evaluates a block's mask expressions and writes the results into its
    /// parameters, so everything downstream sees plain values and never has to
    /// know a mask was involved.
    ///
    /// Done top-down: a block's expressions are resolved against the scope
    /// that encloses it, and a subsystem's freshly resolved parameters become
    /// the scope for its own contents. That is what lets a mask parameter be
    /// passed down through nested custom blocks.
    ...
    void walk(Model& model, Scope* parent, Block* subsystem,
        ...
        // Ahead of everything else in this scope: a parameter can decide how
        // many ports a block has, and pruneDanglingConnections() is already
        ...
        for (const BlockPtr& held : model.blocks()) {
            ...
            // Wireless links are boundary markers at every level: a Goto is
            // an alias for the signal feeding it, and a From is an alias for
            // that same signal, so neither is a block to execute.
    ...
    bool resolveSource(Block* block, int port, Scope* scope, int& outBlock,
                       int& outPort) const {
        // A wireless link can be chained; a chain that closes on itself has no
        // signal in it at all, so catch it here rather than letting the hop
        // limit turn it into a silently unconnected input.
        ...
        for (int hop = 0; hop <= kMaxDepth * 2; ++hop) {
            if (isFrom(block)) {
                ...
                if (!wire) return false;  // nothing drives the Goto
    ...
    std::map<std::string, std::string> globalPath_;  ///< for error messages
