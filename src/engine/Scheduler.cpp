...
/// Tarjan's strongly connected components, iterative so that a deep diagram
/// cannot overflow the stack.
///
/// Components come out in reverse topological order of the condensation,
/// which is exactly what the execution plan needs once reversed.
...
CompiledModel Scheduler::compile(Model& model,
    ...
    auto runSetup = [&](CompiledBlock& cb, BlockSetup& setup) {
        ...
        for (std::size_t p = 0; p < inputs; ++p) {
            ...
            if (width <= 0) continue;  // upstream not resolved yet
    ...
    std::map<int, int> producerOf;  // signal id -> block index
    for (const CompiledBlock& cb : compiled.blocks)
