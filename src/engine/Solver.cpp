...
/// Second order, L-stable, with an embedded first-order estimate.
///
/// Explicit methods have to keep h inside the stability region, which for a
/// stiff model means a step far smaller than accuracy alone would need. This
/// one is unconditionally stable, so the step size follows the solution
/// rather than the fastest decaying mode — at the cost of solving a nonlinear
/// system per stage.
///
/// The tableau, with gamma = 1 - 1/sqrt(2):
///
///     gamma | gamma      0
///         1 | 1-gamma    gamma
///     ------+------------------
///           | 1-gamma    gamma     (order 2, stiffly accurate)
///           | 1          0         (order 1, for the error estimate)
...
public:
    ...
    StepOutcome step(const DerivativeFn& f, double t, Vec& x, double h,
        ...
        double factor = 5.0;
        if (errorNorm > 1e-12)
            factor = kSafety * std::pow(errorNorm, -1.0 / 3.0);  // order 2
...
private:
    static constexpr double kGamma = 0.2928932188134524756;  // 1 - 1/sqrt(2)
    ...
    /// Weighted norm: a component counts against its own tolerance, so one
    /// large state cannot mask an error in a small one.
