...
void DiagramScene::duplicateSelection() {
    ...
    // Deliberately not through the clipboard: duplicating something should not
    // throw away whatever the user copied earlier.
    const std::string text = ModelSerializer::copySelection(*model_, ids);
...
WireItem* DiagramScene::selectedWire() const {
    ...
    for (QGraphicsItem* item : selectedItems()) {
        ...
        if (found) return nullptr;  // more than one; nothing to rename
...
void DiagramScene::drawBackground(QPainter* painter, const QRectF& rect) {
    ...
    // Fade the grid out when zoomed far away, so it never turns into mush.
...
void DiagramScene::finishPendingWire(const QPointF& scenePos) {
    ...
    const PortHit target = portAt(scenePos);
