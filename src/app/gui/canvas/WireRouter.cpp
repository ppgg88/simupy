...
QPainterPath WireRouter::route(const QPointF& from, const QPointF& to,
    ...
    for (const QRectF& rect : obstacles) {
        ...
        // The clearance ring around a block reaches past the point where this
        // wire leaves or enters a port. Where it does, give up the ring for
        // that one block rather than the block itself — dropping it from the
        // obstacle set would let the route cut straight across it, which is
        // exactly what a wire must never do to its own source or target.
    ...
    using Entry = std::pair<qreal, int>;  // (priority, node)
    std::priority_queue<Entry, std::vector<Entry>, std::greater<Entry>> open;

    // The wire leaves its port horizontally, so it starts already moving that
    // way and pays no bend for the first straight run.
