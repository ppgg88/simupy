...
constexpr qreal kPortLength = 7.0;   ///< how far the arrow sticks out
constexpr qreal kPortHalfWidth = 4.5;
constexpr qreal kPortGrab = 11.0;    ///< click radius around a port
...
void BlockItem::refresh() {
    ...
    // Grow the body so ports never collide.
...
void BlockItem::paintPorts(QPainter* painter) const {
    ...
    for (int i = 0; i < outputs_.size(); ++i) {
        ...
        // Output stubs are drawn as a short line with a dot, so the two ends
        // of a wire never look interchangeable.
...
void BlockItem::paintGlyph(QPainter* painter, const QRectF& body) const {
    ...
    // A library block draws whatever its author chose. This comes first so a
    // custom block never falls through to the generic "here is my type name"
    // box at the bottom.
    ...
    if (type == "Gain") {
        ...
        drawFittedText(painter, textRect,
                       formatNumberList(params.vector("gain", {1.0}), 2), 10);
        painter->restore();
        return;
    }
    ...
    if (type == "Sum") {
        ...
        const QString signs =
            QString::fromStdString(params.text("signs", "++"));
        QFont font = painter->font();
        font.setPointSize(9);
        font.setBold(true);
        painter->setFont(font);
    ...
    if (type == "TransferFcn" || type == "DiscreteTransferFcn") {
        ...
        const QString numerator =
            polynomialText(params.vector("numerator", {1.0}), variable);
        const QString denominator =
            polynomialText(params.vector("denominator", {1.0, 1.0}), variable);
    ...
    if (type == "Constant") {
        ...
        drawFittedText(painter, body,
                       formatNumberList(params.vector("value", {1.0})), 12);
        painter->restore();
        return;
    }
    ...
    if (type == "Slider") {
        ...
        const double value =
            interactive ? interactive->liveValue() : params.real("value", 0.0);
    ...
    if (type == "Goto" || type == "From") {
        ...
        QString tag = QString::fromStdString(params.text("tag", ""));
        if (tag.isEmpty()) tag = QStringLiteral("?");
        const bool global = params.text("scope", "local") == "global";
    ...
    if (type == "Inport" || type == "Outport") {
        ...
        painter->save();
        drawFittedText(painter, body,
                       QString::number(params.integer("portNumber", 1)), 14);
        painter->restore();
        return;
    }
    ...
    if (type == "PythonFunction") {
        ...
        painter->setPen(colors.blockText);
        QString className =
            QString::fromStdString(params.text("className", ""));
...
void BlockItem::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    if (event->button() == Qt::LeftButton &&
        ...
        // Accepted without the base class, so the press never starts a
        // block drag: a slider that moves its own block is useless.
        drivingControl_ = true;
        setSelected(true);
        driveControl(event->pos(), true);
        // Accepted without calling the base class, so the press never starts
        // a block drag: a slider you cannot slide without moving the block
        // would be worse than no slider at all.
        event->accept();
        return;
    }
