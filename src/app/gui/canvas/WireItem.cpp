...
WireItem::WireItem(QString connectionId, BlockItem* source, int sourcePort,
    ...
    setZValue(-1.0);  // wires pass behind blocks
...
void WireItem::setSignalName(const QString& name) {
    ...
    // The label sits outside the stroked path, so the item has to be told its
    // extent changed or the old text stays on screen.
...
QRectF WireItem::boundingRect() const {
    ...
    // Room for the width badge above the wire and the name below it.
