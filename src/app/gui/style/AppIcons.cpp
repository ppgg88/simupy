...
/// Strokes `path` with the part `mask` covers removed.
///
/// This is how one shape passes in front of another. The obvious alternative —
/// painting the background colour behind the front shape — would be wrong
/// here: these icons appear on the toolbar, in menus and on the canvas, and
/// those are three different backgrounds. Subtracting leaves real
/// transparency, so the gap shows whatever is actually behind.
...
/// Builds once and remembers. The key is the icon's name, so two callers
/// asking for the same icon share one engine.
