...
class Block:
    ...
    zero_crossings = 0
    #: Set to False when :meth:`output` ignores its inputs, to break loops.
...
class HardwareBlock(Block):
    ...
    def terminate(self):
        ...
        try:
            ...
            # Raising here would replace whatever actually went wrong.
...
class ArduinoBoard:
    ...
    def acquire(cls, port, baud=115200, rate_hz=200):
        ...
        board._users += 1
        if board._users == 1:
            # Forget the previous run's pins, or a pin nothing reads still streams.
            board._reset_inputs()
        return board
    ...
    def __init__(self, port, baud, rate_hz):
        ...
        self._link = serial.Serial(port=port, baudrate=baud, timeout=0.2,
                                   write_timeout=0.5)
        # Native-USB boards reset when the port opens; wait for the sketch's banner.
        self._running = True
    ...
    def _read_loop(self):
        # Buffered here: pyserial can return a line split across two timeouts.
        pending = b""
        ...
        while self._running:
            ...
            pending += chunk
            # A line that never terminates means the far end speaks another protocol.
            if len(pending) > 8192:
                pending = b""
                continue
    ...
    def _parse_config_ack(self, line):
        ...
        with self._lock:
            ...
            # Only the newest configuration can still produce samples.
    ...
    def _parse_sample(self, line):
        ...
        with self._lock:
            ...
            # A sample in flight when the pins changed would pair the wrong values.
