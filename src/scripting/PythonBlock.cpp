...
py::object viewOf(const double* data, Eigen::Index size, bool writable) {
    if (data == nullptr || size <= 0) {
        // A stateless block has a null state buffer, and a capsule cannot
        // hold a null pointer. An empty array needs no backing memory anyway.
...
// pybind11 types have hidden visibility; matching it here keeps the linker
// from warning about a default-visibility struct holding them.
...
py::object resolveBlockClass(const std::string& source,
    ...
    for (const auto& item : namespaceDict) {
        ...
        // Only classes the script defines. A base imported from simupy —
        // HardwareBlock — would otherwise make every script ambiguous.
...
/// Turns the free-form parameter text into a dict for the block instance.
