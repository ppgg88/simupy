...
/// Names a Python block reserves; a mask using them would overwrite its source.
...
void registerCustomBlock(const CustomBlockDef& def) {
    ...
    // Captured by value, so editing a definition cannot mutate live models.
...
void LibraryManager::registerLibrary(CustomLibrary& library) {
    ...
    for (CustomBlockDef& def : library.blocks) {
        ...
        if (BlockRegistry::instance().find(def.name)) {
            // Shadowing a built-in would change what existing models mean.
            continue;
        }
