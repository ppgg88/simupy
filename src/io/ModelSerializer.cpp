...
std::vector<std::string> ModelSerializer::pasteInto(Model& model,
    ...
    // Ids are rewritten before decoding rather than after, so the wires in the
    // document already point at the new blocks and nothing has to be patched
    // up once they exist. Only the top level is remapped: a subsystem's inner
    // ids are scoped to its own diagram and cannot collide with anything here.
