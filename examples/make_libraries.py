...
class UdpSend(HardwareBlock):
    ...
    def output(self, t, u):
        # Sinks must not have side effects in output(): a variable-step solver
        # calls it at trial states it may throw away, and a datagram cannot be
        # taken back. update() is only called on accepted steps.
        return None
    ...
    def update(self, t, u):
        ...
        try:
            ...
            # A send that fails is worth knowing about, but not worth ending
            # a run over — the far end may simply not be listening yet.
...
class UdpReceive(HardwareBlock):
    ...
    def output(self, t, u):
        # Drained rather than read once: under load several datagrams may be
        # waiting, and the freshest is the only one that matters.
        while True:
            try:
...
class Ros2Publish(HardwareBlock):
    ...
    def open_device(self):
        ...
        # The executor runs on its own thread so callbacks and timers are
        # serviced while the solver is between steps.
        self.rclpy = rclpy
        self.running = True
    ...
    def close_device(self, device):
        self.running = False
        device.destroy_node()
        # Deliberately not rclpy.shutdown(): another block may still be using
        # the same context, and tearing it down under them would be worse.
'''



# ----------------------------------------------------------------------
# Arduino / ESP32 — one shared link per board, see simupy.ArduinoBoard
# ----------------------------------------------------------------------
...
class ArduinoAnalogRead(ArduinoBlock):
    ...
    def output(self, t, u):
        # Touching self.device is what opens the link and registers the pin,
        # and it has to happen here: setup() runs several times per compile.
        board = self.device
...
class ArduinoAnalogWrite(ArduinoBlock):
    ...
    def output(self, t, u):
        # Sinks must not act in output(): a variable-step solver calls it at
        # trial states it may discard, and a pin cannot be un-driven.
        return None

    def update(self, t, u):
        duty = float(u[0][0]) / self.scale if self.scale else 0.0
        duty = 0.0 if duty < 0.0 else (1.0 if duty > 1.0 else duty)
        # A quantised comparison, so a signal that is merely noisy does not
        # fill the link with commands the board cannot tell apart anyway.
...
class ArduinoDigitalWrite(ArduinoBlock):
    ...
    def zero_crossing(self, t, u):
        # Declared so the solver lands a step on the switching instant rather
        # than crossing it blind, which is what makes the pin change at the
        # time the model says it does.
