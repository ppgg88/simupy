...
def pid_control_loop():
    ...
    # The measurement runs right to left, so the feedback block is mirrored.
...
'''
    ...
    b.add("ctrl", "PythonFunction", "PI (Python)", -280, 0, width=120,
          ...
          parameters="kp = 3.0\nki = 8.0\nlimit = 5.0")
    # A first-order motor: a PI controller already adds the integrator, so a
    # second one in the plant would make the loop needlessly oscillatory.
