...
class FakeBoard:
    ...
    def __init__(self, board="fake", adc_bits=10, pwm_bits=8, mode="sine"):
        ...
        self.stream = []          # [(pin, is_analog)] in the host's order
        self.generation = 0
        self.rate_hz = 200
        self.digital = {}         # pin -> 0/1 written by the host
        self.pwm = {}             # pin -> duty 0..1 written by the host
        self.modes = {}
    ...
    def read_analog(self, pin):
        if self.mode == "echo":
            # Analog input n mirrors PWM output n, so a model that writes and
            # reads back closes a loop through the "board".
            return int(round(self.pwm.get(pin, 0.0) * self.adc_max))
        t = time.monotonic() - self.started
        # A 0.2 Hz sine offset to the middle of the range, and each pin a
        # quarter turn behind the last so several traces are told apart.
        phase = 0.25 * pin
    ...
    def command(self, line):
        ...
        if verb in "Cc":
            ...
            self.stream = pins
            self.generation += 1
            # Acknowledged, so the host learns the number rather than assuming
            # this board started counting when it did.
...
def serve(board, verbose=False, seconds=0.0):
    ...
    # A fresh pty echoes and translates newlines, so the board would read back
    # everything it wrote and answer its own banner with "unknown command"
    # forever. Raw mode makes it behave like the serial link it stands in for.
